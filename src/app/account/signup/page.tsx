...
export default function AccountSignupPage() {
  ...
  const { signUpWithEmail, signInWithGoogle, isReady } = useAuth();
      ...
      <section className="pt-20 pb-20 bg-[var(--background)] min-h-screen">
        <div className="container mx-auto px-6 max-w-md">
          <div className="text-center mb-10">
            ...
            <h1 className="text-4xl md:text-5xl font-serif text-[var(--foreground)] mt-4">Create Account</h1>
            <p className="text-sm text-[var(--muted)] mt-3 font-light">
              Already a member?{' '}
              <Link href={`/account/login${next ? `?next=${next}` : ''}`} className="text-primary underline-offset-4 hover:underline">
                Sign in
              </Link>
            </p>
          </div>
          ...
          {!isReady && (
            <div className="bg-amber-50 border border-amber-200 text-amber-700 text-xs p-4 mb-6">
              Firebase is not configured on this environment. Add credentials to <code>.env.local</code> to enable sign-up.
            </div>
          )}
          ...
          <form onSubmit={handleEmail} className="space-y-5">
            ...
            <div>
              ...
              <p className="text-[10px] text-[var(--muted)] mt-2">Minimum 6 characters.</p>
            </div>
            <button
              type="submit"
              disabled={loading}
          ...
          <p className="text-[10px] text-center text-[var(--muted)] mt-8 leading-relaxed">
