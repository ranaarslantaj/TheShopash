  ...
  return (
    <div className="space-y-12">
      <header>
        ...
        <h1 className="text-4xl font-serif text-white">Business Overview</h1>
      </header>

      {/* Stats Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((stat) => (
          <div key={stat.name} className="bg-white/5 border border-white/10 p-6 space-y-4">
            <div className="flex justify-between items-start">
              <span className="text-[10px] uppercase tracking-widest text-white/40">{stat.name}</span>
              <stat.icon className="w-4 h-4 text-primary" />
            </div>
            <div className="text-2xl font-serif text-white">{stat.value}</div>
            <p className="text-[10px] text-white/20 uppercase tracking-tighter">{stat.detail}</p>
          </div>
        ))}
      </div>

      {/* Recent Orders */}
      <section className="bg-white/5 border border-white/10 overflow-hidden">
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-white/5">
              {recentOrders.length === 0 ? (
                 <tr>
                    <td colSpan={5} className="px-8 py-12 text-center text-white/20 font-serif">
                        No orders recorded yet.
                    </td>
                 </tr>
              ) : recentOrders.map((order) => (
                <tr key={order.id} className="hover:bg-white/[0.02] transition-colors">
                  <td className="px-8 py-5 text-xs text-white/50 tracking-tighter font-mono">#{order.id.slice(0, 8)}</td>
